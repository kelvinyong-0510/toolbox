  ...
  try {
    ...
    if (method === 'POST') {
      ...
      const result = await db.prepare(
        ...
        name, version || '', icon_emoji || '', icon_bg || 'linear-gradient(135deg,#1B2A4A,#2a3f6e)',
        category || 'utilities', category_label || '', is_new ? 1 : 0, description || '',
        features_json || '[]', download_label || 'Download', download_url || '',
        versions_json || '[]', docs_url || '#', docs_label || 'Docs', sort_order || 0
      ).run();
      return json({ success: true, id: result.meta ? result.meta.last_row_id : null });
    }
    ...
    if (method === 'PUT') {
      ...
      await db.prepare(
        ...
        category || 'utilities', category_label || '', is_new ? 1 : 0, description || '',
        features_json || '[]', download_label || 'Download', download_url || '',
        versions_json || '[]', docs_url || '#', docs_label || 'Docs', sort_order || 0, id
      ).run();
      return json({ success: true });
    }
